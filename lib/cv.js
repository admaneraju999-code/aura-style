...
export async function analyzeImage(buffer) {
  ...
  // Shoes: tall narrow items, usually dark or leather-toned
  if (aspect < 0.6 && height > width * 1.5 && saturation < 0.35) {
    category = "shoes";
    subCategory = "footwear";
    confidence = 0.72;
  }
  // Accessories: small, often square or very wide, low edge density
  else if (aspect > 1.8 && edgeDensity < 0.08) {
    category = "accessories";
    subCategory = "bag";
    confidence = 0.60;
  }
  else if (aspect > 1.3 && edgeDensity < 0.05 && brightness < 100) {
    category = "accessories";
    subCategory = "belt";
    confidence = 0.55;
  }
  // Outerwear: wider than tall, medium-high brightness, often structured edges
  ...
  // Bottoms: wider aspect, lower saturation (denim/wool), specific vertical profile
  else if (aspect > 1.2 && saturation < 0.3 && botBright < midBright) {
    category = "bottoms";
    subCategory = saturation < 0.1 ? "trousers" : "jeans";
    confidence = 0.70;
  }
  else if (aspect > 1.1 && aspect < 1.5 && edgeDensity < 0.07) {
    category = "bottoms";
    subCategory = "skirt";
    confidence = 0.55;
  }
